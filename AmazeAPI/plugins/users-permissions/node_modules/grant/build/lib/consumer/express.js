...
module.exports = function (_config) {
  ...
  var transport = function transport(provider, res, session) {
  ...
  function connect(req, res) {
    ...
    if (provider.oauth === 1) {
      oauth1.request(provider).then(function (_ref) {
        var body = _ref.body;

        session.request = body;
        oauth1.authorize(provider, body).then(function (url) {
  ...
  app.get('/connect/:provider/callback', function (req, res) {
    ...
    if (provider.oauth === 1) {
      oauth1.access(provider, session.request, req.query).then(response).catch(response);
    } else if (provider.oauth === 2) {
      oauth2.access(provider, req.query, session).then(response).catch(response);
    } else {
